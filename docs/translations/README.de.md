...
**Mehragenten-Plugin vorerst selbst verwalten.** Die von HolyCode verwaltete Installation von oh-my-openagent ist in v1.1.4 ausgesetzt. Beim ersten erfolgreichen Start ohne das alte Flag wird der frühere verwaltete Plugin-Eintrag deaktiviert; Einstellungen, Skills und Paket-Cache bleiben erhalten.
...
Das Compose-Setup verwendet das Chromium-seccomp-Profil von HolyCode. Wenn du nicht aus einem Repository-Klon arbeitest, lade zuerst die Release-Datei herunter:
...
- Getaggte Releases aktualisieren OpenCode und die Tool-Pins für dich.
...
| `ENABLE_OH_MY_OPENAGENT` | (keiner) | Veraltete Einstellung; `true` beendet den Start ohne Änderungen, danach deaktiviert eine einmalige Migration den alten aktiven Eintrag |
...
| `ENABLE_HERMES` | (keiner) | Veraltete Einstellung; HolyCode beendet den Start mit einem Migrationshinweis, solange Hermes nicht gebündelt ist |
| `HOLYCODE_PLUGIN_UPDATE` | `manual` | Plugin-Aktualisierungsmodus: `manual` (installiert nur fehlende Plugins und lässt Benutzer-Versionen unverändert) oder `auto` (synchronisiert deklarierte Pins beim Start) |

> `ENABLE_CLAUDE_AUTH` wird beim Neustart des Containers wirksam. `ENABLE_OH_MY_OPENAGENT` ist nur noch eine veraltete Einstellung und beendet derzeit den Start mit einem Migrationshinweis.

> `HOLYCODE_PLUGIN_UPDATE` steuert Plugin-Paketaktualisierungen. `manual` (Standard) installiert aktivierte Plugins nur wenn sie fehlen und behält vom Benutzer installierte Versionen bei. `auto` installiert fehlende Plugins und synchronisiert die deklarierten Pins beim Start. Dies ist unabhängig von `OPENCODE_DISABLE_AUTOUPDATE`, das nur OpenCode betrifft.

> HolyCode installiert oh-my-openagent derzeit nicht. Wenn `ENABLE_OH_MY_OPENAGENT=true` in einer älteren Installation bestehen bleibt, stoppt der Container ohne Änderungen. Nach dem Entfernen des Flags deaktiviert der erste erfolgreiche Start den alten aktiven Eintrag und erhält Einstellungen, Skills und Paket-Cache.
...
> Hermes ist im aktuellen Image weiterhin nicht gebündelt. Wenn `ENABLE_HERMES=true` aus einer älteren Installation bestehen bleibt, beendet HolyCode den Start mit einem kurzen Migrationshinweis. `/home/opencode/.hermes` bleibt unverändert erhalten, damit die Daten nach einer späteren Wiederaufnahme der Integration oder bei einem Rollback verfügbar sind.
...
Paperclip bleibt als optionaler Dienst über OpenCode gebündelt. Die CLIProxyAPI-Integration verwendet weiterhin einen extern verwalteten Endpunkt. Hermes ist vorübergehend nicht im Image enthalten.

### Hermes Agent (vorübergehend nicht gebündelt)

Hermes bleibt aus dem Image entfernt. HolyCode startet keinen Hermes-Prozess und veröffentlicht keinen Hermes-Port.

Deine vorhandenen Daten unter `/home/opencode/.hermes` werden weder gelöscht noch migriert. Entferne `ENABLE_HERMES=true` aus älteren Konfigurationen, damit das aktuelle Image startet. Bewahre das Verzeichnis für eine spätere Wiederaufnahme der Integration oder für die Wiederherstellung eines unveränderten Vorab-Snapshots auf.
...
Der Paperclip-Zustand liegt unter `/home/opencode/.paperclip`. Paperclip läuft ebenfalls mit `/home/opencode` als Home und hält OpenCode-Konfiguration, Cache und Status unter demselben Verzeichnis, sodass die Agenten dieselbe persistente Konfiguration sehen wie die OpenCode-Web-UI. Paperclip liefert den Skills-Katalog jetzt über das Paketset aus, das HolyCode installiert, sodass die Skills-Seite den Katalog lädt statt bei `GET /api/skills/catalog` zu scheitern. Öffne das Dashboard, richte dein Unternehmen ein und stelle dort OpenCode-gestützte Mitarbeiter ein.
...
### oh-my-openagent Migration

HolyCode installiert oder aktualisiert oh-my-openagent in v1.1.4 nicht. Mit `ENABLE_OH_MY_OPENAGENT=true` stoppt der Start, ohne den Plugin-Zustand zu ändern. Nach dem Entfernen des Flags deaktiviert der erste erfolgreiche Start den alten Eintrag, speichert dessen Paketangabe und erhält Einstellungen, Skills und Paket-Cache. Fügst du das Plugin später manuell wieder hinzu, behandelt HolyCode es als benutzerverwaltet.
...
Stoppe den Container und sichere die Home-, Cache- und Workspace-Volumes, bevor du das neueste Image herunterlädst.
...
v1.2.2 behält Paperclip 2026.831.1 bei, daher gibt es keine neue Paperclip-Migration und keine neue Standardüberschreibung für den nativen Runner. Stelle für ein Rollback die unveränderten Home-, Cache- und Workspace-Volumes von vor dem Upgrade mit Image `1.2.1` wieder her; verwende keine Volumes, die bereits mit v1.2.2 gestartet wurden.
...
Paperclip wird in v1.1.4 von 2026.707.0 auf 2026.722.0 migriert. Ein Rollback ist nur mit Image `1.1.3` und unveränderten Volumes von vor dem v1.1.4-Upgrade möglich. Starte Image `1.1.3` niemals mit bereits von v1.1.4 migrierten Paperclip-Daten.
...
<details>
<summary><strong>Wie verwendet HolyCode die Chromium-Sandbox?</strong></summary>

Chromium läuft als Benutzer `opencode` mit aktivierter integrierter Sandbox. Die Compose-Dateien laden das eingeschränkte Profil `config/chromium-seccomp.json` automatisch.

Bei einem eigenen Container-Aufruf muss dasselbe Profil geladen werden:
...
Deaktiviere die Sandbox nicht und gewähre dem Container keine zusätzlichen Berechtigungen.
