...
**Per ora gestisci direttamente il plugin multi-agente.** L'installazione di oh-my-openagent gestita da HolyCode è sospesa nella v1.1.4. Il primo avvio riuscito senza la vecchia impostazione disattiva la precedente voce gestita e conserva impostazioni, Skills e cache del pacchetto.
...
La configurazione Compose usa il profilo seccomp Chromium di HolyCode. Se non stai lavorando da un clone del repository, scarica prima la copia della release:
...
- Le release taggate aggiornano OpenCode e i pin degli strumenti per te.
...
| `ENABLE_OH_MY_OPENAGENT` | (nessuna) | Impostazione precedente; `true` interrompe l'avvio senza modifiche, poi una migrazione unica disattiva la vecchia voce attiva dopo la rimozione |
...
| `ENABLE_HERMES` | (nessuna) | Impostazione precedente; HolyCode interrompe l'avvio con un avviso di migrazione finché Hermes non è incluso |
...
> `ENABLE_CLAUDE_AUTH` ha effetto al riavvio del container. `ENABLE_OH_MY_OPENAGENT` ora è un'impostazione precedente e attualmente interrompe l'avvio con un avviso di migrazione.
...
> HolyCode attualmente non installa oh-my-openagent. Se una vecchia installazione mantiene `ENABLE_OH_MY_OPENAGENT=true`, il container si arresta senza modificare lo stato del plugin. Dopo la rimozione dell'impostazione, il primo avvio riuscito disattiva la vecchia voce attiva e conserva impostazioni, Skills e cache del pacchetto.
...
> Hermes resta non disponibile nell'immagine attuale. Se una vecchia installazione mantiene `ENABLE_HERMES=true`, HolyCode interrompe l'avvio con un breve avviso di migrazione. `/home/opencode/.hermes` resta invariata per un futuro ripristino dell'integrazione o per tornare a uno snapshot precedente.
...
> La v1.2.2 usa OpenCode 1.18.31, OpenSpec 1.13.1, Claude Code 2.1.276, Paperclip 2026.831.1 con Undici 6.28.1, npm 12.0.2, pnpm 12.4.2, ESLint 10.10.0, Prettier 3.9.8, Wrangler 4.134.0 con Miniflare 5.20260917.0-alpha e workerd 1.20260917.1, Prisma 7.10.0, TypeScript 6.0.3 e json-server 0.17.4. Python include Playwright 1.63.0, pandas 3.0.6, Matplotlib 3.11.2, tqdm 4.70.1, FastAPI 0.141.1, Uvicorn 0.53.0 e NumPy 2.5.3. `opencode-claude-auth` 2.2.0 è incluso nell'immagine e viene installato offline all'avvio. Netlify CLI e il pacchetto npm `serve` restano esclusi. L'installazione di oh-my-openagent gestita da HolyCode è sospesa, Hermes resta non disponibile e gli endpoint CLIProxyAPI gestiti esternamente restano supportati.
...
Paperclip resta incluso come servizio opzionale sopra OpenCode. L'integrazione CLIProxyAPI continua a usare un endpoint gestito esternamente. Hermes non è temporaneamente presente nell'immagine.

### Hermes Agent (temporaneamente non incluso)

Hermes resta escluso dall'immagine. HolyCode non avvia processi Hermes e non pubblica la relativa porta.

I dati esistenti in `/home/opencode/.hermes` non vengono eliminati né migrati. Rimuovi `ENABLE_HERMES=true` dalle vecchie configurazioni per avviare l'immagine corrente. Conserva la directory per un futuro ripristino dell'integrazione o per ripristinare uno snapshot precedente rimasto intatto.
...
### Migrazione di oh-my-openagent

HolyCode non installa né aggiorna oh-my-openagent nella v1.1.4. Con `ENABLE_OH_MY_OPENAGENT=true`, l'avvio si interrompe senza modificare lo stato del plugin. Dopo la rimozione dell'impostazione, il primo avvio riuscito disattiva la vecchia voce, ne registra il riferimento del pacchetto e conserva impostazioni, Skills e cache. Se aggiungi di nuovo il plugin manualmente, HolyCode lo considera gestito da te.
...
Arresta il container ed esegui il backup dei volumi home, cache e workspace prima di scaricare l'ultima immagine.
...
La v1.2.2 mantiene Paperclip 2026.831.1, quindi non aggiunge una nuova migrazione Paperclip né modifica il valore predefinito del runner nativo. Per tornare indietro, ripristina i volumi home, cache e workspace intatti creati prima dell'aggiornamento con l'immagine `1.2.1`; non riutilizzare volumi già avviati con la v1.2.2.
...
Nella v1.1.4 Paperclip migra dalla 2026.707.0 alla 2026.722.0. Per tornare indietro usa l'immagine `1.1.3` con volumi intatti creati prima dell'aggiornamento alla v1.1.4. Non avviare mai l'immagine `1.1.3` con dati Paperclip già migrati dalla v1.1.4.
...
<details>
<summary><strong>Come usa HolyCode la sandbox di Chromium?</strong></summary>

Chromium viene eseguito come utente `opencode` con la sandbox integrata attiva. I file Compose caricano automaticamente il profilo limitato `config/chromium-seccomp.json`.

Se avvii il container direttamente, carica lo stesso profilo:
...
Non disattivare la sandbox e non concedere privilegi aggiuntivi al container.
