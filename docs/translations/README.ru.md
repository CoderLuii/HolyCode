...
| `API_SERVER_KEY` | (нет) | Задайте его перед включением Hermes; используйте настоящий Bearer token |
...
|---------|---------|
| Node.js | 24.18.0 (LTS) |
| npm | 11.16.0, поставляется вместе с Node.js 24.18.0 |
| Python | 3 (системный) |
...
> В v1.1.1 OpenCode обновлён до 1.18.1, Claude Code до 2.1.210, s6-overlay до 3.2.3.1, pnpm до 11.13.0, tsx до 4.23.1, а oh-my-openagent до 4.18.1. TypeScript остаётся на 6.0.3, NumPy на 2.4.6, а Vercel на 54.21.0. Netlify поддерживает только удалённые сборки и развёртывания. Встроенный sidecar CLIProxyAPI удалён; внешние управляемые endpoint по-прежнему поддерживаются.
...
Задайте `API_SERVER_KEY` перед включением Hermes.
