...
Состояние Paperclip хранится в `/home/opencode/.paperclip`. Paperclip также запускается с `/home/opencode` как home и держит конфигурацию, кэш и состояние OpenCode в этом же каталоге, поэтому агенты видят те же сохранённые настройки, что и веб-интерфейс OpenCode. HolyCode также кладёт опубликованный каталог Skills от Paperclip по пути, который ожидает стабильная версия, поэтому страница Skills загружает каталог вместо ошибки `GET /api/skills/catalog`. Откройте панель управления, настройте компанию и наймите сотрудников OpenCode оттуда.
