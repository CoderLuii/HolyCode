...
> В v1.2.0 используются OpenCode 1.18.30, Claude Code 2.1.268, Paperclip 2026.831.1 с Undici 6.28.1, npm 12.0.2, pnpm 12.4.0, ESLint 10.10.0, Wrangler 4.131.0, Prisma 7.10.0, TypeScript 6.0.3 и json-server 0.17.4. В Python входят tqdm 4.70.0, FastAPI 0.141.1, Uvicorn 0.52.4 и NumPy 2.5.3. `opencode-claude-auth` 2.2.0 включён в образ и устанавливается без сети при запуске. Netlify CLI и npm-пакет `serve` по-прежнему не включены. Установка oh-my-openagent средствами HolyCode приостановлена, Hermes остаётся недоступным, а внешние управляемые endpoints CLIProxyAPI по-прежнему поддерживаются.
...
В v1.2.0 сохраняется Paperclip 2026.831.1, поэтому новых миграций Paperclip нет. Для отката восстановите нетронутые домашние, кэш- и рабочие тома, созданные до обновления, с образом `1.1.9`; не используйте повторно тома, которые уже запускались с v1.2.0.
