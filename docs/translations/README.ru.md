...
**Пока управляйте мультиагентным плагином самостоятельно.** Установка oh-my-openagent средствами HolyCode приостановлена в v1.1.4. Первый успешный запуск без старой настройки отключает прежнюю управляемую запись плагина, сохраняя настройки, Skills и кэш пакета.
...
Конфигурация Compose использует профиль seccomp Chromium из HolyCode. Если вы запускаете не из клона репозитория, сначала скачайте файл из релиза:
...
| `ENABLE_OH_MY_OPENAGENT` | (нет) | Устаревшая настройка; `true` останавливает запуск без изменений, после удаления одноразовая миграция отключает прежнюю активную запись |
...
| `ENABLE_HERMES` | (нет) | Устаревшая настройка; HolyCode останавливает запуск с сообщением о миграции, пока Hermes не входит в образ |
| `HOLYCODE_PLUGIN_UPDATE` | `manual` | Режим обновления плагинов: `manual` (устанавливает только недостающее и сохраняет версии пользователя) или `auto` (синхронизирует объявленные pin'ы при запуске) |

> `ENABLE_CLAUDE_AUTH` вступает в силу при перезапуске контейнера. `ENABLE_OH_MY_OPENAGENT` теперь является устаревшей настройкой и сейчас останавливает запуск с сообщением о миграции.

> `HOLYCODE_PLUGIN_UPDATE` управляет обновлениями пакетов плагинов. `manual` (по умолчанию) устанавливает включённые плагины только если они отсутствуют и сохраняет версии пользователя. `auto` устанавливает отсутствующие плагины и синхронизирует объявленные pin'ы при запуске. Это отдельно от `OPENCODE_DISABLE_AUTOUPDATE`, который влияет только на OpenCode.

> HolyCode сейчас не устанавливает oh-my-openagent. Если в старой установке осталось `ENABLE_OH_MY_OPENAGENT=true`, контейнер останавливается, не изменяя состояние плагина. После удаления настройки первый успешный запуск отключает прежнюю активную запись и сохраняет настройки, Skills и кэш пакета.
...
> Hermes остаётся недоступным в текущем образе. Если в старой установке осталось `ENABLE_HERMES=true`, HolyCode останавливает запуск и выводит короткое сообщение о миграции. `/home/opencode/.hermes` сохраняется без изменений для будущего возврата интеграции или восстановления предыдущего снимка.
...
| npm | 12.0.2 |
| Python | 3.13 (Trixie) |
| pip | Поставляется с Python 3.13 |

> Теги релизов используют ровно `vX.Y.Z`. Docker-теги опускают `v`. После `v1.0.9` идёт `v1.1.0`, после `v1.1.9` идёт `v1.2.0`, а после `v1.9.9` идёт `v2.0.0`. Версии от `v1.0.10` до `v1.0.13` остаются неизменными.

> В v1.2.2 используются OpenCode 1.18.31, OpenSpec 1.13.1, Claude Code 2.1.276, Paperclip 2026.831.1 с Undici 6.28.1, npm 12.0.2, pnpm 12.4.2, ESLint 10.10.0, Prettier 3.9.8, Wrangler 4.134.0 с Miniflare 5.20260917.0-alpha и workerd 1.20260917.1, Prisma 7.10.0, TypeScript 6.0.3 и json-server 0.17.4. В Python входят Playwright 1.63.0, pandas 3.0.6, Matplotlib 3.11.2, tqdm 4.70.1, FastAPI 0.141.1, Uvicorn 0.53.0 и NumPy 2.5.3. `opencode-claude-auth` 2.2.0 включён в образ и устанавливается без сети при запуске. Netlify CLI и npm-пакет `serve` по-прежнему не включены. Установка oh-my-openagent средствами HolyCode приостановлена, Hermes остаётся недоступным, а внешние управляемые endpoints CLIProxyAPI по-прежнему поддерживаются.
...
Paperclip по-прежнему входит в образ как опциональный сервис поверх OpenCode. Интеграция CLIProxyAPI использует внешний управляемый endpoint. Hermes временно отсутствует в образе.

### Hermes Agent (временно не входит в образ)

Hermes по-прежнему не входит в образ. HolyCode не запускает процесс Hermes и не публикует его порт.

Существующие данные в `/home/opencode/.hermes` не удаляются и не мигрируют. Удалите `ENABLE_HERMES=true` из старой конфигурации, чтобы запустить текущий образ. Сохраните этот каталог для будущего возврата интеграции или восстановления неизменённого предыдущего снимка.
...
Состояние Paperclip хранится в `/home/opencode/.paperclip`. Paperclip также запускается с `/home/opencode` как home и держит конфигурацию, кэш и состояние OpenCode в этом же каталоге, поэтому агенты видят те же сохранённые настройки, что и веб-интерфейс OpenCode. Paperclip теперь поставляет каталог Skills через пакетный набор, который устанавливает HolyCode, поэтому страница Skills загружает каталог вместо ошибки `GET /api/skills/catalog`. Откройте панель управления, настройте компанию и наймите сотрудников OpenCode оттуда.
...
### Миграция oh-my-openagent

HolyCode не устанавливает и не обновляет oh-my-openagent в v1.1.4. При `ENABLE_OH_MY_OPENAGENT=true` запуск останавливается без изменения состояния плагина. После удаления настройки первый успешный запуск отключает прежнюю запись, сохраняет обозначение пакета, настройки, Skills и кэш. Если затем добавить плагин вручную, HolyCode считает его управляемым пользователем.
...
Остановите контейнер и сохраните резервные копии домашнего, кэш- и рабочего томов до загрузки последнего образа.

При обновлении с версии старше `v1.1.3` скачайте указанный выше профиль seccomp и добавьте `security_opt` в сервис `holycode` до пересоздания контейнера.

В v1.2.2 сохраняется Paperclip 2026.831.1, поэтому новых миграций Paperclip и переопределения значения native runner по умолчанию нет. Для отката восстановите нетронутые домашние, кэш- и рабочие тома, созданные до обновления, с образом `1.2.1`; не используйте повторно тома, которые уже запускались с v1.2.2.

В v1.1.9 Paperclip мигрирует с 2026.824.1 на 2026.831.1 через миграции `0223`–`0230`. Устаревшие поля `brandColor` и `attachmentMaxBytes` удаляются, а незавершённые сеансы входа сбрасываются; войдите снова после обновления. Для отката восстановите нетронутую резервную копию с образом `1.1.8`. Никогда не запускайте `1.1.8` с базой данных, уже мигрированной v1.1.9.

В v1.1.4 Paperclip мигрирует с 2026.707.0 на 2026.722.0. Для отката используйте образ `1.1.3` и неизменённые тома, созданные до обновления до v1.1.4. Никогда не запускайте образ `1.1.3` с данными Paperclip, уже мигрированными v1.1.4.
...
<details>
<summary><strong>Как HolyCode использует песочницу Chromium?</strong></summary>

Chromium запускается от пользователя `opencode` со включённой встроенной песочницей. Файлы Compose автоматически загружают ограниченный профиль `config/chromium-seccomp.json`.

При самостоятельном запуске контейнера загрузите тот же профиль:
...
Не отключайте песочницу и не выдавайте контейнеру дополнительные привилегии.
